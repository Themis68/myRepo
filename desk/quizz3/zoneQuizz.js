...
function gestChrono(phase, data) {
	...
	switch(phase) {
		...
		case "R":	// data = numéro de la qproposition cliquée
			...
			let numBonneProp = parseInt(window.question.reponse.solution, 10);
			...
			if (data != "") {
				numPropClic = parseInt(data.substring(data.length -1), 10); // le chiffre correspond à la propositioon cliquée
			} else {
			...
			for (let i=1; i <= 4; i++) {
				if(i == numBonneProp) {
					// bonne réponse
					if(i == numPropClic) {
						//il y a eu clic sur la bonne réponse
						// son
					...
					// couleurs
					fond = "var(--fond-proposition-bonne)";
					bord = "var(--bord-proposition-bonne) solid 1em";
				} else {
					// autre réponse
					if(i == numPropClic) {
						//il y a eu clic sur la mauvaise réponse
						// son
						document.getElementById('mauvaise').play();
						bord = "var(--bord-proposition-mauvaise) solid 1em";
					} else {
						bord = "var(--bord-proposition) solid 1em";
					}
					fond = "var(--fond-proposition)";
				}

				// couleur bordure poroposition (LI)
				document.getElementById("prop" + i).style.borderLeft = bord;
				// fond proposition (H5)
				document.getElementById("libProp" + i).style.backgroundColor = fond;
				// on retire la gestion du click (H5)
				document.getElementById("libProp" + i).removeAttribute("onclick");
			}
